...
const Sidebar: React.FC<SidebarProps> = ({ activeSection, onNavigate, collapsed = false, toggleCollapse }) => {
  ...
  return (
    <aside 
      className={`
        h-full bg-slate-900 dark:bg-slate-950 text-slate-300 flex flex-col border-r border-slate-800 shadow-2xl relative z-20 
    ...
    >
      {/* Brand Header */}
      <div className={`
        h-24 flex items-center border-b border-slate-800/50 bg-slate-950 transition-all duration-300 relative
      ...
      `}>
        <div className="flex items-center gap-4 text-white overflow-hidden">
          ...
          <span className={`text-xl font-bold tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-white to-slate-400 whitespace-nowrap transition-opacity duration-300 ${collapsed ? 'opacity-0 w-0 hidden' : 'opacity-100'}`}>
            Nexus
          </span>
        </div>
        ...
        {toggleCollapse && (
          <button 
            ...
            className={`
              hidden lg:flex items-center justify-center w-6 h-6 rounded-full bg-slate-800 text-slate-400 hover:text-white hover:bg-indigo-600 transition-all absolute 
              ${collapsed ? '-right-3 top-20 shadow-md border border-slate-700' : 'right-4'}
            `}
          >
        ...
        <div className={`flex items-center gap-3 ${collapsed ? 'justify-center' : ''}`}>
           ...
           {!collapsed && (
             <div className="overflow-hidden">
               ...
               <p className="text-xs text-indigo-400 truncate">{user?.role} • {user?.department}</p>
             </div>
           )}
        </div>
      </div>
      ...
      <nav className="flex-1 px-3 space-y-2 overflow-y-auto overflow-x-hidden">
        <p className={`px-4 text-xs font-bold text-slate-600 uppercase tracking-wider mb-2 ${collapsed ? 'hidden' : 'block'}`}>Menu</p>
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              <span className={`font-medium text-sm tracking-wide whitespace-nowrap transition-all duration-300 ${isActive ? 'text-white' : 'text-slate-400 group-hover:text-white'} ${collapsed ? 'opacity-0 w-0 overflow-hidden absolute' : 'opacity-100 relative'}`}>
                {item.label}
              </span>
            </button>
          );
      ...
      {/* Footer Actions */}
      <div className="p-4 space-y-2 border-t border-slate-800/50 bg-slate-900/50 backdrop-blur-sm">
        <button 
           onClick={() => onNavigate(NavSection.SETTINGS)}
           title={collapsed ? t("nav.settings") : ''}
           className={`w-full flex items-center gap-4 py-3 rounded-xl transition-colors ${collapsed ? 'justify-center px-0' : 'px-3 lg:px-4'} ${activeSection === NavSection.SETTINGS ? 'bg-slate-800 text-white' : 'text-slate-400 hover:text-white hover:bg-slate-800/50'}`}
        >
          <Settings size={20} />
        ...
        <button 
          ...
          className={`w-full flex items-center gap-4 py-3 text-slate-400 hover:text-rose-400 hover:bg-rose-500/10 transition-all rounded-xl ${collapsed ? 'justify-center px-0' : 'px-3 lg:px-4'}`}
        >
          <LogOut size={20} />
